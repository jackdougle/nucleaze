...
fn get_reference_kmers(
    ...
    let temp_idx: Vec<FxHashSet<u64>> = (0..num_idx).map(|_| FxHashSet::default()).collect();
    let processor_idx = replace(&mut processor.ref_kmers, temp_idx);
    let processor_idx = Arc::new(processor_idx.into_iter().map(|s| Mutex::new(s)).collect());

    let (sender, receiver) = bounded::<Vec<Vec<u8>>>(16);

    read_chunks(ref_path, sender).expect("K-mer extraction failed...");
    ...
    (0..num_threads).into_par_iter().for_each(|_| {
        ...
        while let Ok(chunk) = receiver.recv() {
            ...
            transfer_chunks(&mut local_idx, &processor_idx);
        }
    ...
    processor.ref_kmers.par_iter_mut().enumerate().for_each(|(i, final_idx)| {
        let vec_idx = processor_idx[i].lock().unwrap();
...
fn transfer_chunks(chunks: &mut Vec<Vec<u64>>, merged: &Vec<Mutex<FxHashSet<u64>>>) {
    for (i, chunk) in chunks.iter_mut().enumerate() {
        if !chunk.is_empty() {
            ...
            {
                let mut set = merged[i].lock().unwrap();
...
fn read_chunks(path: &str, sender: Sender<Vec<Vec<u8>>>) -> Result<(), Box<dyn Error>> {
    ...
    spawn(move || {
        ...
        let chunk_size = 2000;
        let mut chunk: Vec<Vec<u8>> = Vec::with_capacity(2000);
